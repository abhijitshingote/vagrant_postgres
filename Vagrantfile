...
sudo cat << EOF >> /etc/postgresql/9.3/main/pg_hba.conf 
...
echo "trying other version of creating vagrant role"
...
Vagrant.configure("2") do |config|
  ...
  config.vm.network "forwarded_port", guest: 5432, host: 5556
  config.vm.network "forwarded_port", guest: 80, host: 8080
  # config.vm.provision "shell", path: "somescript.sh"
  config.vm.provision "shell", inline: $script
  # config.trigger.after :up do |trigger|
  #     trigger.info = "Running Script"
  #     trigger.run = {path: "somescript.sh"}
  #   end
  # Create a forwarded port mapping which allows access to a specific port
  # within the machine from a port on the host machine and only allow access
  # via 127.0.0.1 to disable public access
  # config.vm.network "forwarded_port", guest: 5432, host: 5556, host_ip: "127.0.0.1"
